...
private[scalaphash] object MathUtils {
    ...
    implicit class FloatMatrixOps(val matrix: FloatMatrix) extends AnyVal {
        def *(other : FloatMatrix) : FloatMatrix = {
            if (matrix.length == 0 || other.length == 0 || other(0).length == 0 || matrix(0).length != other.length) {
        ...
        */
        def crop(x1 : Int, x2 : Int, y1 : Int, y2 : Int) : FloatMatrix = {
            Array.tabulate(x2 - x1 + 1, y2 - y1 + 1)(
                (i, j) => matrix(i + x1)(j + y1)
            )
        }
    }

    implicit class FloatOps(val x : Float) extends AnyVal {
        def ~=(y : Float, delta : Float = 0.0000001f) : Boolean = {
            y - delta <= x && x <= y + delta 
        }
    }
